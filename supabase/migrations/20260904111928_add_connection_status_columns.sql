...
- `sender_status`: 'ready' (default) → 'ack' (sender saw receiver) → 'chunks_ready' (Supabase relay upload done)

Both default to their initial values so existing rows (if any) are treated
as "just published".
*/
